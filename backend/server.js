...
// ✅ Evaluate Answer and Provide Score, Feedback, and Correct Answer
app.post("/evaluate-answer", async (req, res) => {
    try {
        const { question, answer } = req.body;

        const promptText = `Evaluate the following user answer for the given interview question. Provide a score out of 10, detailed feedback on what was done well, where the user can improve, and finally, provide a complete, correct answer. Format the response clearly with a Score:, Feedback:, and Correct Answer: section.
        ...
        let evaluation = response.data.response?.trim();
        if (!evaluation) {
            return res.status(500).json({ error: "Failed to evaluate answer" });
        }

        // Add the response to userResponses for potential later use
        userResponses.push({ question, answer, evaluation });
